...
void FGPUResourceMaterial::SetVector(const FString& Name, const FVector4& Value)
{
...
void FGPUResourceMaterial::SetTexture(const FString& Name, const TSharedPtr<const FGPUResourceRenderTarget>& Value)
{
	if (Value)
	{
		MaterialInstance->SetTextureParameterValue(FName(*Name), Value->GetNativeRenderTarget());
	}
	else
	{
